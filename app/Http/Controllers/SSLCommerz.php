...
class SSLCommerz
{
    ...
    public function orderValidate($trx_id = '', $amount = 0, $currency = "TL", $post_data)
    {
...
//                 }
                ...
                if ($code == 200 && !(curl_errno($handle))) {
                    ...
                    if ($status == "VALID" || $status == "VALIDATED") {
                        if ($merchant_trans_currency == "TL") {
                            if (trim($merchant_trans_id) == trim($tran_id) && (abs($merchant_trans_amount - $amount) < 1) && trim($merchant_trans_currency) == trim('TL')) {
                                return true;
                            } else {
                                # DATA TEMPERED
